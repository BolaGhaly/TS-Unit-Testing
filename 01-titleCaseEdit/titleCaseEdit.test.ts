import { titleCaseEdit } from "./titleCaseEdit";
...
describe("the titleCaseEdit(title) function", function () {
  ...
  it("should return a string of the same length as the input title", function () {
    ...
    expect(
      submittedSolution,
      "the output possesses a length differing from the length of the input title"
    ).to.have.lengthOf(4);
  });

  it("should pass auxillary test cases", function () {
    let auxillaryTestCases = [
      "the sun also rises",
      "east Of Eden",
      "a scanner Darkly",
      "The Invisible Man",
    ...
    expect(submittedSolution(auxillaryTestCases[0])).to.equal(
      "The Sun Also Rises"
    );
    expect(submittedSolution(auxillaryTestCases[1])).to.equal("East Of Eden");
    expect(submittedSolution(auxillaryTestCases[2])).to.equal(
      "A Scanner Darkly"
    );
    expect(submittedSolution(auxillaryTestCases[3])).to.equal(
      "The Invisible Man"
    );

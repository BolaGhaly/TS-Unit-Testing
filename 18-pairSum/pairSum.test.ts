...
describe("the pairSum(nums, target) function", function () {
  it("should throw an error if the length of `nums` array is 1 or lower", function () {
    ...
    expect(function () {
      submittedSolution([], 9000);
    }).to.throw();
    expect(function () {
      submittedSolution([9], 9000);
    }).to.throw();
  });
  ...
  it("should not throw an error if the length of `nums` array is 2 or greater", function () {
    let submittedSolution = pairSum;
    expect(function () {
      submittedSolution([1, 2], 9000);
    }).to.not.throw();
    expect(function () {
      submittedSolution([1, 2, 3], 9000);
    }).to.not.throw();
  });

  it("should return a boolean value", function () {
    let submittedSolution = pairSum([1, 2, 3], 4);
    expect(submittedSolution, "the output is not a boolean value").to.be.a(
      "boolean"
    );
  });

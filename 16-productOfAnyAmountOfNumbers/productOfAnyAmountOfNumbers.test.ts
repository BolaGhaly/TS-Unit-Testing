import { productOfAnyAmountOfNumbers } from "./productOfAnyAmountOfNumbers";
...
describe("the productOfAnyAmountOfNumbers(...args) function", function () {
  ...
  it("should return the product of all of the numbers that were passed in as arguments", function () {
    ...
    expect(
      submittedSolution,
      "the output does not reflect an accurate product"
    ).to.equal(490);
  });
  ...
  it("should pass auxillary test cases", function () {
    ...
    expect(
      submittedSolution(randomNumberA, randomNumberB, randomNumberC)
    ).to.equal(randomNumberA * randomNumberB * randomNumberC);
  });
});
